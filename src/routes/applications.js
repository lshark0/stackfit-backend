...
router.post('/jobs/:id/apply', requireAuth, requireRole('freelancer'), (req, res) => {
  const jobId = Number(req.params.id);
  const job = get('SELECT * FROM jobs WHERE id = ?', [jobId]);
  if (!job) return res.status(404).json({ error: '공고를 찾을 수 없습니다.' });

  const existing = get('SELECT id FROM applications WHERE job_id=? AND freelancer_id=?', [jobId, req.user.id]);
  if (existing) return res.status(409).json({ error: '이미 지원한 공고입니다.' });

  run('INSERT INTO applications (job_id, freelancer_id) VALUES (?,?)', [jobId, req.user.id]);

  const profile = get('SELECT name FROM freelancer_profiles WHERE user_id = ?', [req.user.id]);
...
router.get('/me/applications', requireAuth, requireRole('freelancer'), (req, res) => {
  const rows = all(
...
router.get('/jobs/:id/applicants', requireAuth, requireRole('company'), (req, res) => {
  const job = get('SELECT * FROM jobs WHERE id = ? AND company_id = ?', [req.params.id, req.user.id]);
  if (!job) return res.status(404).json({ error: '공고를 찾을 수 없습니다.' });
  const rows = all(
...
router.patch('/jobs/:jobId/applicants/:applicationId', requireAuth, requireRole('company'), (req, res) => {
  const { status } = req.body || {};
  if (!['accepted', 'rejected'].includes(status)) {
    return res.status(400).json({ error: "status는 'accepted' 또는 'rejected'여야 합니다." });
  }
  const job = get('SELECT * FROM jobs WHERE id = ? AND company_id = ?', [req.params.jobId, req.user.id]);
  if (!job) return res.status(404).json({ error: '공고를 찾을 수 없습니다.' });

  const application = get('SELECT * FROM applications WHERE id = ? AND job_id = ?', [req.params.applicationId, job.id]);
  if (!application) return res.status(404).json({ error: '지원 내역을 찾을 수 없습니다.' });

  run('UPDATE applications SET status = ? WHERE id = ?', [status, application.id]);

  const freelancer = get('SELECT name FROM freelancer_profiles WHERE user_id = ?', [application.freelancer_id]);
  run('INSERT INTO notifications (user_id, tag, title, body) VALUES (?,?,?,?)', [
  ...
  if (status === 'accepted') {
    const existingProject = get('SELECT id FROM projects WHERE job_id=? AND freelancer_id=?', [job.id, application.freelancer_id]);
    if (!existingProject) {
      run(
