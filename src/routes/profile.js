...
router.get('/', requireAuth, (req, res) => {
  if (req.user.role === 'freelancer') {
    const p = get('SELECT * FROM freelancer_profiles WHERE user_id = ?', [req.user.id]);
    return res.json({
      ...p,
  ...
  const c = get('SELECT * FROM companies WHERE user_id = ?', [req.user.id]);
  res.json(c);
});
...
router.put('/', requireAuth, (req, res) => {
  if (req.user.role === 'freelancer') {
    ...
    const current = get('SELECT * FROM freelancer_profiles WHERE user_id = ?', [req.user.id]);
    ...
    const updated = get('SELECT * FROM freelancer_profiles WHERE user_id = ?', [req.user.id]);
    return res.json({ ...updated, stack: JSON.parse(updated.stack_json) });
  }

  const { name, contact_person, description } = req.body || {};
  const current = get('SELECT * FROM companies WHERE user_id = ?', [req.user.id]);
  run('UPDATE companies SET name=?, contact_person=?, description=? WHERE user_id=?', [
    name ?? current.name,
    contact_person ?? current.contact_person,
    description ?? current.description,
    req.user.id,
  ]);
  res.json(get('SELECT * FROM companies WHERE user_id = ?', [req.user.id]));
});
...
router.post('/resume', requireAuth, requireRole('freelancer'), (req, res) => {
  upload.single('resume')(req, res, (err) => {
    ...
    run('UPDATE freelancer_profiles SET resume_filename=?, resume_original_name=?, completion=? WHERE user_id=?', [
...
router.delete('/resume', requireAuth, requireRole('freelancer'), (req, res) => {
  const current = get('SELECT * FROM freelancer_profiles WHERE user_id = ?', [req.user.id]);
  if (current.resume_filename) {
    fs.unlink(path.join(UPLOAD_DIR, current.resume_filename), () => {});
  }
  run('UPDATE freelancer_profiles SET resume_filename=NULL, resume_original_name=NULL WHERE user_id=?', [req.user.id]);
