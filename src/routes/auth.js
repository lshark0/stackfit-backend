...
router.post('/signup', async (req, res) => {
  ...
  if (role === 'freelancer') {
    await run('INSERT INTO freelancer_profiles (user_id, name) VALUES (?,?)', [userId, name || '이름 미입력']);
  } else {
    await run('INSERT INTO companies (user_id, name) VALUES (?,?)', [userId, companyName || '회사명 미입력']);
  }
